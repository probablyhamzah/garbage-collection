...
void markPhase(Node* ptr)
{
    ...
    while (!done)
    {
        ...
        if (ptr->numFields != 0 && ind < ptr->numFields)
        {
            curr = ptr->fields[ind];
            ptr->fields[ind] = prev;
            prev = ptr;
        ...
        {
            Node* temp = prev;
            prev = prev->fields[ind];
            temp->fields[ind] = curr;
            curr = temp;
            ptr = curr;
            
            ind++;
            if (prev == NULL && ind >= ptr->numFields)
                done = 1;
        }
    }
}
...
int main()
{
    ...
    ptrs[1]->fields[0] = ptrs[2];
    ptrs[1]->fields[1] = ptrs[9];
    ptrs[1]->fields[2] = ptrs[10];
    ptrs[1]->numFields = 3;
    ptrs[1]->data = 1;

    ptrs[2]->data = 2;
    
    ptrs[3]->fields[0] = ptrs[8];
    ptrs[3]->fields[1] = ptrs[10];
    ptrs[3]->numFields = 2;
    ptrs[3]->data = 3;

    
    ptrs[5]->fields[0] = ptrs[1];
    ptrs[5]->numFields = 1;
    ptrs[5]->data = 5;

    
    ptrs[7]->fields[0] = ptrs[1];
    ptrs[7]->fields[1] = ptrs[8];
    ptrs[7]->numFields = 2;
    ptrs[7]->data = 7;
    
    ptrs[8]->fields[0] = ptrs[9];
    ptrs[8]->numFields = 1;
    ptrs[8]->data = 8;
